...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    // Check if the quiz has a totalScore defined
    if (result.totalScore) {
      // Calculate the level based on the user's score and the total possible score
      const level = Math.ceil(1 + (score / result.totalScore) * 5) ?? 1;

      // Update the user's level if it has increased
    ...
    // const score = result?.markedOptions.reduce(
    //   (p, c) => p + (c.correct ? getScore(c.question.difficulty) : 0),
    //   0
    // );
    // const level = Math.ceil(1 + (score / result.totalScore) * 5) ?? 1;
    // await prisma.result.update({
